 ...
 * 
 * (C) 2011 Meetup, Inc.
 * Author: <a href="mailto:andrew@meetup.com">Andrew Gwozdziewycz</a>
 *
 *
 * Example usage:
 *
...
public class StatsdClient extends TimerTask {
    ...
    private synchronized boolean doSend(String stat) {
        try {
            ...
            log.error(String.format("Could not send stat %s to host %s:%d", sendBuffer.toString(), _address.getHostName(), _address.getPort()), e);
            return false;
        }
    }
    ...
    public synchronized boolean flush() {
        try {
            ...
            if (sizeOfBuffer <= 0) { // empty buffer
                return false;
            }
            ...
            if (sizeOfBuffer == nbSentBytes) {
                ...
                log.error(String.format("Could not send entirely stat %s to host %s:%d. Only sent %d bytes out of %d bytes", sendBuffer.toString(),
